...
def train_VoQL_agent_deep_sea_treasure(env_size):
    # Create a Deep Sea Treasure
    game = DeepSeaTreasure(width=env_size, seed=100, speed=1000)
...
def train_multi_objective_dqn_agent(is_linear=True, extended_config=True):
    if extended_config:
        ...
        environment = FruitEnvironment(
            game, max_episode_steps=60, state_processor=AtariProcessor()
        )
    else:
        # Create a Deep Sea Treasure game
        game = DeepSeaTreasure(
    ...
    agent = AgentFactory.create(
        ...
        log_dir="./train/deep_sea_treasure/mo_dqn_checkpoints",
    )
